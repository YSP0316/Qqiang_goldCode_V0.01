...
int main()
{
	...
	//gold[1000]存放历史金价，g1~g5 周一~周五的金价,u1~5周一到周五各日的历史增长和，uu[i]存放连续增长i后继续增长的天次数，ud连续增长i后下跌，du连续下跌i日后增长，dd连续下跌i日后继续下跌，
	...
	//week[1000],存放星期值；	j,i,无意义变量；k,数据组序号变量；	n,数据组长度； n1,n2,n3,n4,n5,对应星期一~五的总天数；judge,涨落标志位；cn,连续涨落的天数；uod,判断是佛正确标志；test; 
...
/********************计算结果概率************************/
	...
	pslope=0;
	pup=0;
	////////////通过第一层 三个属性给出10组的概率 求出第二层的一组三因素的概率///////////////////////////////////////////
	for(i=0;i<10;i++)      
	...
	/////////////通过第二层数据（一组三因素概率）求助结果数据pup，上涨概率///////////////////////////////
...
/****************************调整权重****************************************/
	...
	if(week[n-1]==1) n1=n1+1;//最后一个数据是周一。周一天数加一
	if(week[n-1]==2) n2=n2+1;
	if(week[n-1]==3) n3=n3+1;
	if(week[n-1]==4) n4=n4+1;
	if(week[n-1]==5) n5=n5+1;
